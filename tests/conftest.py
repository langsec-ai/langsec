...
from langsec.schema.sql.enums import JoinType, AggregationType, Access
...
def security_schema_allow_all():
    ...
    default_column_schema = ColumnSchema(
        access=Access.READ, allowed_aggregations={AggregationType.SUM, AggregationType.COUNT}
    )

    return SecuritySchema(
        default_table_security_schema=default_table_schema, default_column_security_schema=default_column_schema
    )
...
def security_guard_deny_AVG():
    ...
    default_table_schema = TableSchema(allowed_joins={}, default_allowed_join=None)

    default_column_schema = ColumnSchema(access=Access.READ, allowed_aggregations={AggregationType.SUM})
...
def security_guard_deny_all():
    ...
    default_column_schema = ColumnSchema(access=Access.DENIED, allowed_aggregations=set())
...
def security_guard_require_where_clause_all():
    ...
    default_column_schema = ColumnSchema(access=Access.READ, allowed_aggregations=set())
    ...
    return SecuritySchema(
        tables={
            "users": TableSchema(
                ...
                allowed_joins={"orders": {JoinType.INNER, JoinType.LEFT}},
            ...
            "orders": TableSchema(
                ...
                allowed_joins={"users": {JoinType.INNER, JoinType.LEFT}},
                default_allowed_join=None,
            ),
        },
        max_joins=2,
    ...
    return SecuritySchema(
        tables={
            "users": TableSchema(
                ...
                allowed_joins={"orders": {JoinType.INNER, JoinType.LEFT}, "products": {JoinType.INNER, JoinType.LEFT}},
                default_allowed_join=None,
            ),
            "orders": TableSchema(
                columns={
...
def mixed_access_schema():
    ...
    return SecuritySchema(
        tables={
            "users": TableSchema(
                columns={
                    "id": ColumnSchema(access=Access.READ, allowed_operations={"SELECT"}),
                    "username": ColumnSchema(access=Access.READ, allowed_operations={"SELECT"}),
                    "email": ColumnSchema(access=Access.WRITE, allowed_operations={"SELECT", "UPDATE"}),
